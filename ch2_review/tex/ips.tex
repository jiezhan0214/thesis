...
The following part of this section explains \correct{each of the above groups of methodologies with the associated research progress}.
...
To save a snapshot of the system computing state, the system \correct{copies the } current stacks and heaps, local and global variables, general registers, the stack pointer, and the program counter, into the NVM. 
...
After the supply voltage recovers, \correct{the system restores its state from the last checkpoint, and hence, continues its execution from that checkpoint}. 
...
\begin{figure}
    ...
    \caption{Harvest-Store-Use execution (taken from~\cite{hester2017future}~\correct{© 2017 ACM}).}
...
\begin{figure}
    ...
    \caption{Voltage trace with hibernation and restoration points in Hibernus (taken from \cite{balsamo2015hibernus}~\correct{© 2015 IEEE}).}
...
\begin{figure}
    ...
    \caption{Memory architectures of traditional processors and NVPs (taken from~\cite{liu2015ambient}~\correct{© 2015 ACM}).}
    \label{Figure:nvp}
\end{figure}
...
\begin{figure}
    ...
    \caption{Architecture of an example power neutral system based on TI MSP430FR platform (taken from \cite{balsamo2016graceful}~\correct{© 2016 IEEE}).}
...
\begin{figure}
    ...
    \caption{Board power consumption of ODROID XU4 vs operating frequency and core configurations, running CPU intensive application Raytrace (taken from \cite{fletcher2017power}~\correct{© 2017 IEEE}).}
    \label{Figure:dvfs}
\end{figure}
...
Samoyed differs from \correct{proactive } intermittent computing approaches, e.g. Alpaca~\cite{maeng2017alpaca}, mainly on handling computational workloads where it reactively checkpoints when the buffered energy is below a threshold, and supports user-customised subdivision of peripheral operations when the operation cannot complete in one active cycle. 
