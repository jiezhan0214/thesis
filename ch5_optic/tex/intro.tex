...
\begin{enumerate}
    ...
    \item An adaptive thresholding scheme that, utilising the runtime energy profiling method, dynamically allocates barely sufficient energy budgets for tasks, with an optional scaling based on user-defined parameters (Section~\ref{sec:method2}). The proposed scheme enables a system to survive with xx\% less energy buffering capacitance than the initially allocated amount and presents a xx\% speedup with variable data sizes, compared to SoA approaches (Section~\ref{sec:experiment}).
